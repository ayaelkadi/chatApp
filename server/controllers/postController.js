...
import { createdNotification } from "./notificationController.js";
const getUserData = async(userId)=>{
    return await User.findById(userId).select("_id full_name username profile_picture")
    .lean() || {_id:"", full_name:"Unknown", username:"Unknown", profile_picture:""}
}
...
export const addComment = async(req,res)=> {
    try {
        ...
        if(!text.trim()) return res.status({success:false, message:"Comment text is required"})
...
export const getUserPost = async (req,res)=> {
    try {
        const {id} = req.params
        const posts = await Post.ffind({use:id}).sort({createdAt:-1}).populate("user", "id full_name username profile_picture")
